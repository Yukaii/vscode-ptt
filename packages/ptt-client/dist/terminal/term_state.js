    ...
    for (i = 0; i < lines.length; i++) {
        ...
        if (getWidth(stringWidth, lines[i]) > this.columns - c.x) {
            if (this._modes.wrap) {
                ...
                lines.splice(i, 1, substrWidth(stringWidth, lines[i], 0, this.columns - c.x), lines[i].substr(splitWidth));
                wrapped = true;
            }
            else {
                ...
                lines[i] = substrWidth(stringWidth, lines[i], 0, this.columns - c.x - getWidth(stringWidth, lastChar)) +
                    lastChar;
            }
        }
        // write line
...
//# sourceMappingURL=term_state.js.map